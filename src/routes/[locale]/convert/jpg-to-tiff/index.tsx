...
export default component$(() => {
  ...
  const downloadFile = $(async (result: ConversionResult) => {
    try {
      ...
      a.href = url;
      a.download = result.outputFilename || "converted.tiff";
    ...
    try {
      ...
      for (const result of successfulResults) {
        if (result.blob) {
          ...
          zip.file(result.outputFilename || "converted.tiff", arrayBuffer);
        } else if (result.downloadPath) {
          ...
          zip.file(result.outputFilename || "converted.tiff", bytes);
        }
      }
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = `jpg-to-tiff-conversions-${Date.now()}.zip`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
  ...
  const convertFiles = $(async () => {
    if (selectedFiles.value.length === 0) {
      errorMessage.value = "Please select at least one JPG file";
      return;
    }
    ...
    try {
      if (mode.value === "single") {
        ...
        const response = await fetch(`${API_BASE_URL}/convert/jpg-to-tiff/single`, {
          method: "POST",
          body: formData,
        ...
        if (response.ok) {
          ...
          const filename = selectedFiles.value[0].name.replace(/\.(jpg|jpeg)$/i, ".tiff");
        ...
        const response = await fetch(`${API_BASE_URL}/convert/jpg-to-tiff/batch`, {
          method: "POST",
          body: formData,
          signal: controller.signal,
        });
              ...
              <div class="space-y-6">
                <div>
                  <h3 class="text-xl font-bold text-gray-900 mb-3">
                    {conv.about.whatIsTiff.title}
                  </h3>
                  <p class="text-gray-700 leading-relaxed">{conv.about.whatIsTiff.content}</p>
                </div>
...
export const head: DocumentHead = ({ resolveValue, url }) => {
  ...
  const conv = localeData.translations.jpgToTiff;
  ...
  return {
    ...
    meta: [
      ...
      {
        property: "og:image",
        content: "https://formipeek.com/og-jpg-to-tiff.jpg",
      },
      {
        name: "twitter:card",
