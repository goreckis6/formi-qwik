...
export default component$(() => {
  ...
  const downloadAllAsZip = $(async () => {
    ...
    try {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `heic-to-tiff-conversions-${Date.now()}.zip`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
  ...
  const convertFiles = $(async () => {
    ...
    try {
      if (mode.value === 'single') {
        ...
        const response = await fetch(`${API_BASE_URL}/convert/heic-to-tiff/single`, {
          method: 'POST',
          body: formData,
        });
        ...
        if (response.ok) {
          ...
          const filename = selectedFiles.value[0].name.replace(/\.(heic|heif)$/i, '.tiff');
        ...
        const response = await fetch(`${API_BASE_URL}/convert/heic-to-tiff/batch`, {
          method: 'POST',
          body: formData,
        });
...
export const head: DocumentHead = ({ resolveValue }) => {
  ...
  return {
    ...
    meta: [
      ...
      {
        property: "og:image",
        content: "https://formipeek.com/og-heic-to-tiff.jpg",
      },
      // Twitter
      {
        name: "twitter:card",
