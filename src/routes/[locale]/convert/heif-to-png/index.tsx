...
export default component$(() => {
  ...
  const downloadAllAsZip = $(async () => {
    ...
    try {
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = `heif-to-png-conversions-${Date.now()}.zip`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
  ...
  const convertFiles = $(async () => {
    ...
    try {
      if (mode.value === "single") {
        ...
        const response = await fetch(`${API_BASE_URL}/convert/heif-to-png/single`, {
          method: "POST",
          body: formData,
        ...
        if (response.ok) {
          ...
          const filename = selectedFiles.value[0].name.replace(/\.heif$/i, ".png");
        ...
        const response = await fetch(`${API_BASE_URL}/convert/heif-to-png/batch`, {
          method: "POST",
          body: formData,
          signal: controller.signal,
        });
...
export const head: DocumentHead = ({ resolveValue, url }) => {
  ...
  const conv = localeData.translations.heifToPng;
  ...
  return {
    ...
    meta: [
      ...
      {
        property: "og:image",
        content: "https://formipeek.com/og-heif-to-png.jpg",
      },
      {
        name: "twitter:card",
