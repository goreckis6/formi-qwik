...
export default component$(() => {
  ...
  const downloadAllAsZip = $(async () => {
    ...
    try {
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = `jpg-to-gif-conversions-${Date.now()}.zip`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
  ...
  const convertFiles = $(async () => {
    ...
    try {
      if (mode.value === "single") {
        ...
        const response = await fetch(`${API_BASE_URL}/convert/jpg-to-gif/single`, {
          method: "POST",
          body: formData,
        ...
        if (response.ok) {
          ...
          const filename = selectedFiles.value[0].name.replace(/\.(jpg|jpeg)$/i, ".gif");
        ...
        const response = await fetch(`${API_BASE_URL}/convert/jpg-to-gif/batch`, {
          method: "POST",
          body: formData,
          signal: controller.signal,
        });
...
export const head: DocumentHead = ({ resolveValue, url }) => {
  ...
  return {
    ...
    meta: [
      ...
      {
        property: "og:image",
        content: "https://formipeek.com/og-jpg-to-gif.jpg",
      },
      {
        name: "twitter:card",
