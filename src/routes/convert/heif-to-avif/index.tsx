    ...
    try {
      ...
      for (const result of successfulResults) {
        if (result.blob) {
          ...
          zip.file(result.outputFilename || "converted.avif", arrayBuffer);
        } else if (result.downloadPath) {
  ...
  const convertFiles = $(async () => {
    ...
    try {
      if (mode.value === "single") {
        ...
        const response = await fetch(`${API_BASE_URL}/convert/heif-to-avif/single`, {
          method: "POST",
          body: formData,
        ...
        if (response.ok) {
          ...
          const filename = selectedFiles.value[0].name.replace(/\.heif$/i, ".avif");
        ...
        const response = await fetch(`${API_BASE_URL}/convert/heif-to-avif/batch`, {
          method: "POST",
          body: formData,
          signal: controller.signal,
        });
...
export const head: DocumentHead = ({ resolveValue, url }) => {
  ...
  const conv = localeData.translations.heifToAvif;
  ...
  return {
    ...
    meta: [
      ...
      {
        property: "og:image",
        content: "https://formipeek.com/og-heif-to-avif.avif",
      },
      {
        name: "twitter:card",
