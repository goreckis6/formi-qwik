    ...
    try {
      ...
      for (const result of successfulResults) {
        if (result.blob) {
          ...
          zip.file(result.outputFilename || "converted.bmp", arrayBuffer);
        } else if (result.downloadPath) {
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = `jpg-to-bmp-conversions-${Date.now()}.zip`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
    ...
    try {
      if (mode.value === "single") {
        ...
        if (response.ok) {
          ...
          const filename = selectedFiles.value[0].name.replace(/\.(jpg|jpeg)$/i, ".bmp");
...
export const head: DocumentHead = ({ resolveValue, url }) => {
  ...
  return {
    ...
    meta: [
      ...
      {
        property: "og:image",
        content: "https://formipeek.com/og-jpg-to-bmp.jpg",
      },
      {
        name: "twitter:card",
