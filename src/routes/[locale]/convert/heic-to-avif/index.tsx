...
export default component$(() => {
  ...
  const downloadAllAsZip = $(async () => {
    ...
    try {
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = `heic-to-avif-conversions-${Date.now()}.zip`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
  ...
  const convertFiles = $(async () => {
    ...
    try {
      if (mode.value === "single") {
        ...
        const response = await fetch(`${API_BASE_URL}/convert/heic-to-avif/single`, {
          method: "POST",
          body: formData,
        ...
        if (response.ok) {
          ...
          const filename = selectedFiles.value[0].name.replace(/\.(heic|heif)$/i, ".avif");
        ...
        const response = await fetch(`${API_BASE_URL}/convert/heic-to-avif/batch`, {
          method: "POST",
          body: formData,
          signal: controller.signal,
        });
                    ...
                    >
                      {isConverting.value ? (
                        ...
                        <span class="flex items-center justify-center gap-2">
                          ...
                          {conv.upload.download.replace('Download ', 'Convert to ')}
                        </span>
                      )}
                    </button>
                    <button
...
export const head: DocumentHead = ({ resolveValue }) => {
  ...
  return {
    ...
    meta: [
      ...
      {
        property: "og:image",
        content: "https://formipeek.com/og-heic-to-avif.jpg",
      },
      // Twitter
      {
        name: "twitter:card",
