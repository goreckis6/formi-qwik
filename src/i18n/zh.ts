...
export const zh: Translations = {
  ...
  jpgToBmp: {
    title: "JPG 转 BMP 转换器 - 在线免费转换为 Windows 位图",
    metaDescription:
      "立即将 JPG 图片转换为 BMP 格式。免费在线 JPG 转 BMP，支持批量处理。快速、安全、无需注册。适合 Windows 兼容与无压缩高质量。",
    metaKeywords:
      "JPG 转 BMP, JPG 转换 BMP, 图片转 BMP, 照片转 BMP, 批量 JPG 转换, BMP 转换器, 免费 BMP 转换器, Windows 位图",
    hero: {
      title: "JPG 转 BMP 转换器",
      subtitle:
        "立即将 JPG 图片转换为 Windows BMP 格式。BMP（位图）为无压缩图像格式，适合 Windows 兼容并保留最大画质。免费、快速、安全。",
      badge1: "无压缩质量",
      badge2: "Windows 兼容",
      badge3: "无需注册",
    },
    upload: {
      title: "上传 JPG 文件",
      description:
        "将 JPG 图片拖放到此处或点击浏览。支持最多 20 个文件的批量转换。单文件最大 100MB。",
      batchTitle: "将JPG照片拖放到此处或点击浏览",
      batchDescription:
        "支持最多20个文件的批量转换。每个文件最大100MB。",
      buttonSingle: "单个文件",
      buttonBatch: "批量转换",
      chooseFile: "选择文件",
      converting: "转换中...",
    ...
    howItWorks: {
      title: "工作原理",
      step1Title: "上传 JPG 文件",
      step1Desc: "从设备中选择一个或多个 JPG 图片",
      step2Title: "自动转换",
      step2Desc: "我们的转换器会立即处理文件",
      step3Title: "下载 BMP",
      step3Desc: "立即获取转换后的 BMP 文件",
    },
    about: {
      title: "关于 JPG 转 BMP",
      whatIsHeif: {
        title: "什么是 BMP 格式？",
        content:
          "BMP（位图图像文件）是 Microsoft 为 Windows 开发的光栅图像文件格式。BMP 文件不压缩，意味着它们可以保留原始图像的每个像素，而没有任何压缩伪影。虽然 BMP 文件比 JPG 等压缩格式更大，但它们提供最大图像质量，并在 Windows 系统中得到广泛支持。BMP 格式支持各种色深（1 位、4 位、8 位、16 位、24 位和 32 位），非常适合需要未压缩图像数据的应用程序。",
      },
      whyConvert: {
        title: "为什么将 JPG 转为 BMP？",
        content:
          "当你需要未压缩图像质量或 Windows 特定兼容性时，将 JPG 转换为 BMP。JPG 使用有损压缩，这会永久降低图像质量，而 BMP 未压缩，可以完美保留每个像素。转换的关键原因：保持最大图像质量（无压缩伪影）、确保 Windows 兼容性（原生 BMP 支持）、为 Windows 应用程序准备图像（许多 Windows 程序偏好 BMP）、无质量损失地归档图像（未压缩格式），或在旧系统中使用图像（BMP 得到广泛支持）。BMP 对于需要像素完美精度、Windows 软件开发和质量保持至关重要的归档目的至关重要。请注意，由于缺乏压缩，BMP 文件将明显大于 JPG 文件。",
      },
      heifVsHeic: {
        title: "BMP vs JPG - 何时选择 BMP",
        content:
          "BMP 和 JPG 服务于不同目的。选择 BMP 用于：未压缩质量（像素完美保留）、Windows 应用程序（原生支持）、归档存储（无质量损失）、软件开发（Windows 程序）、旧系统（通用兼容性）或像素完美要求（无伪影）。选择 JPG 用于：Web 使用（较小文件大小）、在线共享（更快传输）、社交媒体（原生支持）、一般摄影（高效存储）或文件大小重要时（压缩格式）。BMP 的优势：无质量损失（未压缩）、Windows 原生（内置支持）、像素完美（精确保留）、简单格式（易于处理）。BMP 的缺点：大文件大小（未压缩）、不友好于 Web（加载缓慢）、无压缩选项（存储效率低）。对于 Windows 应用程序：选择 BMP。对于 Web 和共享：使用 JPG。对于无质量损失的归档：BMP 更好。",
      },
      benefits: {
        title: "BMP 格式优势",
        list: [
    ...
    faq: {
      title: "常见问题 – JPG 转 BMP",
      items: [
        {
          q: "JPG 转 BMP 是免费的吗？",
          a: "是的！我们的 JPG 转 BMP 转换器完全免费，转换次数无限制。无需注册、订阅或隐藏费用。可根据 Windows 应用程序或归档目的需要转换任意数量的图像。",
        },
        {
          q: "BMP 文件会比 JPG 文件大吗？",
          a: "是的，BMP 文件通常比 JPG 文件大 3-10 倍，因为 BMP 未压缩，而 JPG 使用有损压缩。但是，BMP 可以完美保留每个像素，无质量损失，使其成为需要最大图像质量的应用程序的理想选择。",
        },
        {
          q: "将 JPG 转换为 BMP 时，图像质量会保持吗？",
          a: "BMP 格式保留 JPG 文件中的图像数据，但由于 JPG 使用有损压缩，原始 JPG 中可能已经丢失了一些质量。转换为 BMP 可防止进一步的质量损失，并完美保持当前图像质量。为获得最佳效果，请从高质量的 JPG 图像开始。",
        },
        {
          q: "我可以在 Windows 以外的系统上使用 BMP 文件吗？",
          a: "是的，BMP 文件可以在 macOS、Linux 和其他操作系统上打开，尽管它们最常与 Windows 关联。大多数图像查看器和编辑器在所有平台上都支持 BMP 格式。",
        },
        {
          q: "JPG 转 BMP 转换的最大文件大小是多少？",
          a: "每个 JPG 文件最多可转换 100MB。对于批量转换，可以一次上传最多 20 个文件，总大小限制为 100MB。请注意，生成的 BMP 文件将大于原始 JPG 文件。",
        },
        {
          q: "为什么选择 BMP 而不是 JPG？",
          a: "当你需要未压缩质量、Windows 特定兼容性，或在使用需要 BMP 格式的应用程序时，选择 BMP。BMP 非常适合归档目的、Windows 软件开发，以及最大图像质量比文件大小更重要的 situations。对于 Web 使用或一般共享，由于文件大小较小，JPG 通常更实用。",
        },
        {
          q: "JPG 转 BMP 转换需要多长时间？",
          a: "转换每张图像需要 1-2 秒。BMP 转换很快，因为它主要是格式更改。结果是准备好用于 Windows 应用程序或归档使用的未压缩 BMP 文件。",
        },
      ],
    },
    related: {
      title: "相关转换器",
      converters: [
        { name: "JPG 转 PNG", url: "/convert/jpg-to-png" },
        { name: "PNG 转 BMP", url: "/convert/png-to-bmp" },
        { name: "BMP 转 PNG", url: "/convert/bmp-to-png" },
      ],
    },
