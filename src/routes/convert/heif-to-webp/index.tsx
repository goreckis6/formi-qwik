...
export default component$(() => {
  ...
  const downloadFile = $(async (result: ConversionResult) => {
    try {
      ...
      a.href = url;
      a.download = result.outputFilename || "converted.webp";
    ...
    try {
      ...
      for (const result of successfulResults) {
        if (result.blob) {
          ...
          zip.file(result.outputFilename || "converted.webp", arrayBuffer);
        } else if (result.downloadPath) {
          ...
          zip.file(result.outputFilename || "converted.webp", bytes);
        }
      }
  ...
  const convertFiles = $(async () => {
    ...
    try {
      if (mode.value === "single") {
        ...
        const response = await fetch(`${API_BASE_URL}/convert/heif-to-webp/single`, {
          method: "POST",
          body: formData,
        ...
        if (response.ok) {
          ...
          const filename = selectedFiles.value[0].name.replace(/\.heif$/i, ".webp");
        ...
        const response = await fetch(`${API_BASE_URL}/convert/heif-to-webp/batch`, {
          method: "POST",
          body: formData,
          signal: controller.signal,
        });
...
export const head: DocumentHead = ({ resolveValue, url }) => {
  ...
  const conv = localeData.translations.heifToWebp;
  ...
  return {
    ...
    meta: [
      ...
      {
        property: "og:image",
        content: "https://formipeek.com/og-heif-to-webp.webp",
      },
      {
        name: "twitter:card",
