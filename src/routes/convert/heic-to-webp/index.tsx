...
export default component$(() => {
  ...
  const downloadAllAsZip = $(async () => {
    ...
    try {
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = `heic-to-webp-conversions-${Date.now()}.zip`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
  ...
  const convertFiles = $(async () => {
    ...
    try {
      if (mode.value === "single") {
        ...
        const response = await fetch(`${API_BASE_URL}/convert/heic-to-webp/single`, {
          method: "POST",
          body: formData,
        ...
        if (response.ok) {
          ...
          const filename = selectedFiles.value[0].name.replace(/\.(heic|heif)$/i, ".webp");
        ...
        const response = await fetch(`${API_BASE_URL}/convert/heic-to-webp/batch`, {
          method: "POST",
          body: formData,
          signal: controller.signal,
        });
                    ...
                    >
                      {isConverting.value ? (
                        ...
                        <span class="flex items-center justify-center gap-2">
                          ...
                          {mode.value === "single" ? "Convert to WebP" : "Convert to WebP"}
                        </span>
                      )}
                    </button>
                    <button
...
export const head: DocumentHead = ({ resolveValue, url }) => {
  ...
  const conv = localeData.translations.heicToWebp;
  ...
  return {
    ...
    meta: [
      ...
      {
        property: "og:image",
        content: "https://formipeek.com/og-heic-to-webp.jpg",
      },
      {
        name: "twitter:card",
