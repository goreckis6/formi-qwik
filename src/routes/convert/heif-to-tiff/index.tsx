...
export default component$(() => {
  ...
  const handleFileSelect = $((event: Event) => {
    ...
    if (input.files && input.files.length > 0) {
      ...
      if (files.length === 0) {
        errorMessage.value = "Please select HEIF files only.";
  ...
  const convertFiles = $(async () => {
    ...
    try {
      if (mode.value === "single") {
        ...
        const response = await fetch(`${API_BASE_URL}/convert/heif-to-tiff/single`, {
          method: "POST",
          body: formData,
        ...
        if (response.ok) {
          ...
          const filename = selectedFiles.value[0].name.replace(/\.heif$/i, ".tiff");
        ...
        const response = await fetch(`${API_BASE_URL}/convert/heif-to-tiff/batch`, {
          method: "POST",
          body: formData,
          signal: controller.signal,
        });
              ...
              >
                ...
                <label for="file-input" class="cursor-pointer">
                  ...
                  <h3 class="text-lg font-semibold text-gray-900 mb-2">
                    {mode.value === "single"
                      ? conv.upload.title
                      : "Drag & drop your HEIF images here or click to browse"}
                  </h3>
...
export const head: DocumentHead = ({ resolveValue, url }) => {
  ...
  const conv = localeData.translations.heifToTiff;
  ...
  return {
    ...
    meta: [
      ...
      {
        property: "og:image",
        content: "https://formipeek.com/og-heif-to-tiff.tiff",
      },
      {
        name: "twitter:card",
